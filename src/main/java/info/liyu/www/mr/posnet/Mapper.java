...
public class Mapper extends org.apache.hadoop.mapreduce.Mapper<LongWritable, Text, Text, Text> {
    ...
    //进行数据格式化操作
    info.xiaohei.www.mr.posnet.LineFormater lineFormater = new info.xiaohei.www.mr.posnet.LineFormater();
    //记录数据是pos还是net类型
    Boolean isPos;
    //要计算的日期
    String time;
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        } catch (info.xiaohei.www.mr.posnet.LineException e) {
            if (e.getFlag() == -1) {
                context.getCounter(info.xiaohei.www.mr.posnet.Counter.OUTOFTIMESKIP).increment(1);
            } else if (e.getFlag() == 0) {
                context.getCounter(info.xiaohei.www.mr.posnet.Counter.TIMESKIP).increment(1);
            } else {
                context.getCounter(info.xiaohei.www.mr.posnet.Counter.OUTOFTIMEFLASGSKIP).increment(1);
            }
        } catch (Exception ex) {
            context.getCounter(info.xiaohei.www.mr.posnet.Counter.LINESKIP).increment(1);
        }
