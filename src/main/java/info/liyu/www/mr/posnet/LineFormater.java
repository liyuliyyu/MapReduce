...
public class LineFormater {
     ...
     * */
    public void format(String line, Boolean isPos, String date, String[] timepoint) throws info.xiaohei.www.mr.posnet.LineException {
        ...
        if (!this.time.startsWith(date)) {
            throw new info.xiaohei.www.mr.posnet.LineException("Incorrect datetime!", -1);
        }
        //将字符串的time字段转成时间类型的day字段,如果该时间格式不正确,则过滤并统计异常
        try {
            this.day = this.simpleDateFormat.parse(this.time);
        } catch (ParseException e) {
            throw new info.xiaohei.www.mr.posnet.LineException("Incorrect date format!", 0);
        }
        ...
        for (int i = 0; i < timepoint.length; i++) {
            if (Integer.parseInt(timepoint[i]) <= hour) {
                try {
                    ...
                    throw new info.xiaohei.www.mr.posnet.LineException("Current hour is bigger than the max-timepoint!", 1);
                }
            } else {
